...
def get_auditoria(client: redis.Redis, checksum: str) -> dict:
    """
    Obtiene la auditoría almacenada por checksum.
    
    Args:
        client: Cliente Redis
        checksum: Checksum de la reserva
    
    Returns:
        dict con la auditoría o None si no existe
    """
    try:
        ...
        if data:
            logger.info(
                f"[{get_bogota_time()}] [INFO] Auditoría encontrada para checksum: {checksum[:16]}..."
            )
            return json.loads(data)
        logger.warning(
            f"[{get_bogota_time()}] [WARN] No se encontró auditoría para checksum: {checksum[:16]}..."
        )
        return None
    except redis.RedisError as e:
        logger.error(
            f"[{get_bogota_time()}] [ERROR] Error al obtener auditoría: {str(e)}"
        )
        return None


def update_auditoria_estado(client: redis.Redis, checksum: str, estado: str, ttl: int = 3600):
    """
    ...
    try:
        ...
        data = client.get(key)
        if data:
            auditoria = json.loads(data)
            auditoria["estado"] = estado
            client.setex(key, ttl, json.dumps(auditoria))
            logger.info(
                f"[{get_bogota_time()}] [INFO] Estado de auditoría actualizado a '{estado}' "
                f"para checksum: {checksum[:16]}..."
            )
    except redis.RedisError as e:
        logger.error(
            f"[{get_bogota_time()}] [ERROR] Error al actualizar auditoría: {str(e)}"
        )
